#include <iostream>
#include <string>
#include <cctype>
#include <algorithm>
#include <cassert>

#include "base64_decode.hpp"

constexpr int base64[256] = {
    ['A'] = 0,  ['B'] = 1,  ['C'] = 2,  ['D'] = 3,
    ['E'] = 4,  ['F'] = 5,  ['G'] = 6,  ['H'] = 7,
    ['I'] = 8,  ['J'] = 9,  ['K'] = 10, ['L'] = 11,
    ['M'] = 12, ['N'] = 13, ['O'] = 14, ['P'] = 15,
    ['Q'] = 16, ['R'] = 17, ['S'] = 18, ['T'] = 19,
    ['U'] = 20, ['V'] = 21, ['W'] = 22, ['X'] = 23,
    ['Y'] = 24, ['Z'] = 25, ['a'] = 26, ['b'] = 27,
    ['c'] = 28, ['d'] = 29, ['e'] = 30, ['f'] = 31,
    ['g'] = 32, ['h'] = 33, ['i'] = 34, ['j'] = 35,
    ['k'] = 36, ['l'] = 37, ['m'] = 38, ['n'] = 39,
    ['o'] = 40, ['p'] = 41, ['q'] = 42, ['r'] = 43,
    ['s'] = 44, ['t'] = 45, ['u'] = 46, ['v'] = 47,
    ['w'] = 48, ['x'] = 49, ['y'] = 50, ['z'] = 51,
    ['0'] = 52, ['1'] = 53, ['2'] = 54, ['3'] = 55,
    ['4'] = 56, ['5'] = 57, ['6'] = 58, ['7'] = 59,
    ['8'] = 60, ['9'] = 61, ['+'] = 62, ['/'] = 63
};

std::string base64_decode(std::string str, bool validate , bool trim) {
    if(trim) str.erase(remove_if(str.begin(), str.end(), isspace), str.end());
    if(validate) {
        for(char c: str)
            assert(c == 'A' || c == '=' || base64[c]);
        assert(str.length() % 4 == 0);
    }

    std::string ret;
    for(int i = 0; i < str.length(); i += 4) {
        ret += (base64[str[i]] << 2) + (base64[str[i+1]] >> 4);
        ret += ((base64[str[i+1]] % (1 << 4)) << 4) + (base64[str[i+2]] >> 2);
        ret += ((base64[str[i+2]] % (1 << 2)) << 6) + base64[str[i+3]];
    }

    if(str.length() > 0) {
        ret.pop_back();
        if(str[str.length()-1] == '=') ret.pop_back();
        if(str[str.length()-2] == '=') ret.pop_back();
    }

    return ret;
}
